...
include
  struct
    ...
    let file_stat_of_sexp =
      (let error_source__014_ = "lib/mach_std.ml.file_stat" in
       fun x__015_ ->
         Sexplib0.Sexp_conv_record.record_of_sexp ~caller:error_source__014_
           ~fields:(Field
                      {
                        name = "mtime";
                        kind = Required;
           ...
           x__015_ : Sexplib0.Sexp.t -> file_stat)
    let _ = file_stat_of_sexp
    let sexp_of_file_stat =
      (fun { mtime = mtime__017_; size = size__019_ } ->
         let bnds__016_ = ([] : _ Stdlib.List.t) in
         let bnds__016_ =
           let arg__020_ = sexp_of_int size__019_ in
...
sig
  type t
  val create : unit -> t[@@ocaml.doc " Create an empty rule builder "]
  val to_list : t -> Build_file_format.t[@@ocaml.doc
                                          " Convert the rule builder to a list of build rules "]
  val add : ?deps:string list -> t -> Cmd.t list -> unit[@@ocaml.doc
                                                          " Add a new rule from a list of commands. "]
  val add_dyndep : ?deps:string list -> t -> Cmd.t list -> unit[@@ocaml.doc
                                                                 " Add a new dyndep rule from a list of commands. "]
end[@@ocaml.doc " A rule builder "]
type t
val create : unit -> t
...
module Build_file_format =
  struct
    type stanza =
      ...
      targets: string array
        [@sexp.array ][@ocaml.doc
                        " absolute path of a target containing dyndep "];
      deps: string array
        [@sexp.array ][@ocaml.doc
                        " absolute paths of dependencies rule requires "];
      commands: string array
      ...
      struct
        ...
        let stanza_of_sexp =
          (let error_source__003_ =
             "lib/mach_build.ml.Build_file_format.stanza" in
           function
           | Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom
               ("rule" | "Rule"))::sexps__006_) as sexp__005_ ->
               Sexplib0.Sexp_conv_record.record_of_sexps ~context:sexp__005_
                 ...
                 ~create:(fun (targets, (deps, (commands, ()))) ->
                            (Rule { targets; deps; commands } : stanza))
                 sexps__006_
           | Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom
               ("rule_dyndep" | "Rule_dyndep"))::sexps__008_) as sexp__007_
               ->
               Sexplib0.Sexp_conv_record.record_of_sexps ~context:sexp__007_
                 ~caller:error_source__003_
                 ~fields:(Field
                            {
                              name = "targets";
                 ...
                 ~index_of_field:(function
                                  ...
                                  | _ -> (-1)) ~allow_extra_fields:false
                 ~create:(fun (targets, (deps, (commands, ()))) ->
                            (Rule_dyndep { targets; deps; commands } : 
                            stanza)) sexps__008_
           | Sexplib0.Sexp.Atom ("rule" | "Rule") as sexp__004_ ->
               Sexplib0.Sexp_conv_error.stag_takes_args error_source__003_
                 sexp__004_
           | Sexplib0.Sexp.Atom ("rule_dyndep" | "Rule_dyndep") as sexp__004_
               ->
               Sexplib0.Sexp_conv_error.stag_takes_args error_source__003_
                 sexp__004_
           ...
           | sexp__002_ ->
               Sexplib0.Sexp_conv_error.unexpected_stag error_source__003_
                 sexp__002_ : Sexplib0.Sexp.t -> stanza)
        let _ = stanza_of_sexp
        let sexp_of_stanza =
          (function
           | Rule
           ...
           | Rule_dyndep
               { targets = targets__024_; deps = deps__028_;
                 commands = commands__032_ }
               ->
               let bnds__022_ = ([] : _ Stdlib.List.t) in
               let bnds__022_ =
                 if match commands__032_ with | [||] -> true | _ -> false
                 then bnds__022_
                 else
                   (let arg__034_ =
                      (sexp_of_array sexp_of_string) commands__032_ in
                    let bnd__033_ =
                      Sexplib0.Sexp.List
                        [Sexplib0.Sexp.Atom "commands"; arg__034_] in
                    (bnd__033_ :: bnds__022_ : _ Stdlib.List.t)) in
               let bnds__022_ =
                 if match deps__028_ with | [||] -> true | _ -> false
                 then bnds__022_
                 else
                   (let arg__030_ = (sexp_of_array sexp_of_string) deps__028_ in
                    let bnd__029_ =
                      Sexplib0.Sexp.List
                        [Sexplib0.Sexp.Atom "deps"; arg__030_] in
                    (bnd__029_ :: bnds__022_ : _ Stdlib.List.t)) in
               let bnds__022_ =
                 if match targets__024_ with | [||] -> true | _ -> false
                 then bnds__022_
                 else
                   (let arg__026_ =
                      (sexp_of_array sexp_of_string) targets__024_ in
                    let bnd__025_ =
                      Sexplib0.Sexp.List
                        [Sexplib0.Sexp.Atom "targets"; arg__026_] in
                    (bnd__025_ :: bnds__022_ : _ Stdlib.List.t)) in
    ...
    type t = stanza list
    let of_string s =
      (Parsexp.Many.parse_string_exn s) |> (List.map ~f:stanza_of_sexp)
    let of_file file_path =
      of_string (let open In_channel in with_open_text file_path input_all)
    let to_string t =
           ...
           fun x__037_ ->
             Sexplib0.Sexp_conv_record.record_of_sexp
               ...
               ~create:(fun (target, (deps, ())) ->
                          ({ target; deps } : dyndep)) x__037_ : Sexplib0.Sexp.t
                                                                   -> 
                                                                   dyndep)
        let _ = dyndep_of_sexp
        let sexp_of_dyndep =
          (fun { target = target__039_; deps = deps__042_ } ->
             let bnds__038_ = ([] : _ Stdlib.List.t) in
             let bnds__038_ =
               if match deps__042_ with | [||] -> true | _ -> false
               then bnds__038_
               else
                 (let arg__044_ = (sexp_of_array sexp_of_string) deps__042_ in
                  let bnd__043_ =
                    Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "deps"; arg__044_] in
                  (bnd__043_ :: bnds__038_ : _ Stdlib.List.t)) in
             let bnds__038_ =
               let arg__040_ = sexp_of_string target__039_ in
               ((Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "target"; arg__040_])
                 :: bnds__038_ : _ Stdlib.List.t) in
             Sexplib0.Sexp.List bnds__038_ : dyndep -> Sexplib0.Sexp.t)
        let _ = sexp_of_dyndep
      end[@@ocaml.doc "@inline"][@@merlin.hide ]
    type t = dyndep list
    let of_string s : t=
      (Parsexp.Many.parse_string_exn s) |> (List.map ~f:dyndep_of_sexp)
    let of_file file_path : t=
      of_string (let open In_channel in with_open_text file_path input_all)
...
let needs_rebuild (rule : rule) =
  ...
  | Some target_mtime ->
      let dep_is_newer dep_path =
        ...
        | None -> true
        | Some stat -> stat.mtime > target_mtime in
      (Array.exists ~f:dep_is_newer rule.deps) ||
        (Array.exists ~f:dep_is_newer rule.dyndeps)
let create () : t= { rules = (T.create 256) }
let configure t (rules : Build_file_format.stanza list) : unit=
  List.iter rules
    ~f:(fun (rule : Build_file_format.stanza) ->
...
let flatten_uniq_list xss =
  (ListLabels.fold_left xss ~init:SS.empty
     ~f:(fun acc xs -> SS.add_seq (List.to_seq xs) acc))
    |> SS.to_list
module Cmd =
  struct
    type t = {
    ...
    let concat' cmds =
      ...
      let commands =
        List.map cmds
          ~f:(fun { command; deps; targets } ->
                all_deps := (deps :: (!all_deps));
                all_targets := (targets :: (!all_targets));
                command) in
      (commands, (flatten_uniq_list (!all_deps)),
        (flatten_uniq_list (!all_targets)))
    let concat cmds =
...
let preprocess_ocaml_module rules cfg ~build_dir ~path_ml ~path_mli ~kind
  ...
  let pp_mlx =
    match kind with
    | Mach_module.ML -> Mach_build.Cmd.v ~targets:[] ~deps:[] ""
    | MLX -> Mach_build.Cmd.v ~targets:[] ~deps:[] " --pp mlx-pp" in
  let pp_ppx =
    match ppx_driver with
    | None -> Mach_build.Cmd.v ~targets:[] ~deps:[] ""
    | Some exe ->
        Mach_build.Cmd.v ~targets:[] ~deps:[exe]
          ((Printf.sprintf " --pp %s") exe) in
  Mach_build.Rule.add rules
    [Mach_build.Cmd.v
       ~targets:(List.flatten
                   [[ml];
                   mach.Mach_build.Cmd.targets;
                   pp_mlx.Mach_build.Cmd.targets;
                   pp_ppx.Mach_build.Cmd.targets])
       ~deps:(List.flatten
                [[path_ml];
                mach.Mach_build.Cmd.deps;
                pp_mlx.Mach_build.Cmd.deps;
                pp_ppx.Mach_build.Cmd.deps])
       ((((((Printf.sprintf "%s pp %s %s -o %s %s")
              mach.Mach_build.Cmd.command) pp_mlx.Mach_build.Cmd.command)
            pp_ppx.Mach_build.Cmd.command) ml) path_ml)];
  (let mli =
     Option.map
       (fun path_mli ->
          ...
          Mach_build.Rule.add rules
            [Mach_build.Cmd.v
               ...
               ~deps:(List.flatten
                        ...
                        mach.Mach_build.Cmd.deps;
                        pp_ppx.Mach_build.Cmd.deps])
               (((((Printf.sprintf "%s pp %s -o %s %s")
                     mach.Mach_build.Cmd.command)
                    pp_ppx.Mach_build.Cmd.command) mli) path_mli)];
          mli) path_mli in
   (ml, mli))
let ocamldep rules cfg ~build_dir ~path_ml ~includes_args =
  let mach = Cmd.v cfg.Mach_config.mach_executable_path in
  let path_dep =
    let open Filename in (build_dir / (modname_of path_ml)) ^ ".dep" in
  Mach_build.Rule.add_dyndep rules
    [Mach_build.Cmd.v
       ~targets:(List.flatten [[path_dep]; mach.Mach_build.Cmd.targets])
       ~deps:(List.flatten
                [[path_ml]; [includes_args]; mach.Mach_build.Cmd.deps])
       (((((Printf.sprintf "%s dep %s -o %s --args %s")
             mach.Mach_build.Cmd.command) path_ml) path_dep) includes_args)];
  path_dep
let compile_ocaml_args ?(include_self= false) rules cfg ~requires ~build_dir
  ...
  let (dirs, libs) =
    List.partition_map
      (function
       | Mach_module.Require r | Mach_module.Require_lib r ->
           Either.Left (Cmd.v (build_dir_of r.v))
       | Mach_module.Require_extlib lib -> Right (Cmd.v (lib.v).name))
      requires in
  let dirs = if include_self then (Cmd.v build_dir) :: dirs else dirs in
  let ocamldep_args =
    let target = let open Filename in build_dir / "ocamldep.args" in
    let cmds =
      match dirs with
      | [] ->
          [Mach_build.Cmd.v ~targets:[target] ~deps:[]
             ((Printf.sprintf "touch %s") target)]
      | _ ->
          List.map
            (fun dir ->
               Mach_build.Cmd.v
                 ~targets:(List.flatten
                             [[target]; dir.Mach_build.Cmd.targets])
                 ~deps:(List.flatten [dir.Mach_build.Cmd.deps])
                 (((Printf.sprintf "echo '-I=%s' >> %s")
                     dir.Mach_build.Cmd.command) target)) dirs in
    let cmds =
      (Mach_build.Cmd.v ~targets:[target] ~deps:(List.flatten [deps])
         ((Printf.sprintf "rm -f %s ") target))
      :: cmds in
    Rule.add rules cmds; target in
  let includes_args =
    let target = let open Filename in build_dir / "includes.args" in
    let cmds =
...
let link_ocaml_executable rules _cfg ~build_dir ~objs:(objs : string list)
  ...
  Mach_build.Rule.add rules
    [Mach_build.Cmd.v ~targets:[objs_args] ~deps:(List.flatten [objs])
       (((Printf.sprintf "printf '%%s\n' %s > %s") (String.concat " " objs))
          objs_args)];
  (match extlibs with
   | [] ->
       Mach_build.Rule.add rules
         [Mach_build.Cmd.v ~targets:[exe_path] ~deps:[objs_args]
...
let link_ocaml_library rules cfg ~build_dir ~cmxs:(cmxs : string list) ~deps
  ~lib_name =
  let mach = Cmd.v cfg.Mach_config.mach_executable_path in
  let link_deps = let open Filename in (build_dir / lib_name) ^ ".link-deps" in
  let cmxa = let open Filename in (build_dir / lib_name) ^ ".cmxa" in
  let cmxa_a = let open Filename in (build_dir / lib_name) ^ ".a" in
  Mach_build.Rule.add rules
    [Mach_build.Cmd.v
       ~targets:(List.flatten [[link_deps]; mach.Mach_build.Cmd.targets])
       ~deps:(List.flatten [deps; mach.Mach_build.Cmd.deps])
       ((((Printf.sprintf "%s link-deps %s > %s") mach.Mach_build.Cmd.command)
           (String.concat " " deps)) link_deps)];
  Mach_build.Rule.add rules
...
include
  struct
    ...
    let mach_lib_mod_of_sexp =
      (let error_source__002_ = "lib/mach_state.ml.mach_lib_mod" in
       fun x__003_ ->
         Sexplib0.Sexp_conv_record.record_of_sexp ~caller:error_source__002_
           ~fields:(Field
                      {
           ...
           ~index_of_field:(function
                            ...
                            | _ -> (-1)) ~allow_extra_fields:false
           ~create:(fun (file_ml, (file_mli, ())) ->
                      ({ file_ml; file_mli } : mach_lib_mod)) x__003_ : 
    ...
    let sexp_of_mach_lib_mod =
      (fun { file_ml = file_ml__005_; file_mli = file_mli__007_ } ->
         ...
         let bnds__004_ =
           ...
           ((Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "file_ml"; arg__006_]) ::
             bnds__004_ : _ Stdlib.List.t) in
         Sexplib0.Sexp.List bnds__004_ : mach_lib_mod -> Sexplib0.Sexp.t)
...
include
  struct
    let _ = fun (_ : extlib) -> ()
    let extlib_of_sexp =
      (let error_source__010_ = "lib/mach_state.ml.extlib" in
       fun x__011_ ->
         Sexplib0.Sexp_conv_record.record_of_sexp ~caller:error_source__010_
           ...
           x__011_ : Sexplib0.Sexp.t -> extlib)
      ...
      (fun { name = name__013_; version = version__015_ } ->
         ...
         let bnds__012_ =
           ...
           ((Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "name"; arg__014_]) ::
             bnds__012_ : _ Stdlib.List.t) in
         Sexplib0.Sexp.List bnds__012_ : extlib -> Sexplib0.Sexp.t)
...
include
  struct
    let _ = fun (_ : require) -> ()
    let require_of_sexp =
      (let error_source__019_ = "lib/mach_state.ml.require" in
       function
       | Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom
           ("require" | "Require" as _tag__022_))::sexp_args__023_) as
           _sexp__021_ ->
           (match sexp_args__023_ with
            | arg0__024_::[] ->
                let res0__025_ = with_loc_of_sexp string_of_sexp arg0__024_ in
                Require res0__025_
            | _ ->
                Sexplib0.Sexp_conv_error.stag_incorrect_n_args
                  error_source__019_ _tag__022_ _sexp__021_)
       | Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom
           ("require_lib" | "Require_lib" as _tag__027_))::sexp_args__028_)
           as _sexp__026_ ->
           (match sexp_args__028_ with
            | arg0__029_::[] ->
                let res0__030_ = with_loc_of_sexp string_of_sexp arg0__029_ in
                Require_lib res0__030_
            | _ ->
                Sexplib0.Sexp_conv_error.stag_incorrect_n_args
                  error_source__019_ _tag__027_ _sexp__026_)
       | Sexplib0.Sexp.List ((Sexplib0.Sexp.Atom
           ("require_extlib" | "Require_extlib" as _tag__032_))::sexp_args__033_)
           as _sexp__031_ ->
           (match sexp_args__033_ with
            | arg0__034_::[] ->
                let res0__035_ = with_loc_of_sexp extlib_of_sexp arg0__034_ in
                Require_extlib res0__035_
            | _ ->
                Sexplib0.Sexp_conv_error.stag_incorrect_n_args
                  error_source__019_ _tag__032_ _sexp__031_)
       | Sexplib0.Sexp.Atom ("require" | "Require") as sexp__020_ ->
           Sexplib0.Sexp_conv_error.stag_takes_args error_source__019_
             sexp__020_
       | Sexplib0.Sexp.Atom ("require_lib" | "Require_lib") as sexp__020_ ->
           Sexplib0.Sexp_conv_error.stag_takes_args error_source__019_
             sexp__020_
       | Sexplib0.Sexp.Atom ("require_extlib" | "Require_extlib") as
           ...
           Sexplib0.Sexp_conv_error.stag_takes_args error_source__019_
             sexp__020_
       | Sexplib0.Sexp.List ((Sexplib0.Sexp.List _)::_) as sexp__018_ ->
           Sexplib0.Sexp_conv_error.nested_list_invalid_sum
             error_source__019_ sexp__018_
       | Sexplib0.Sexp.List [] as sexp__018_ ->
           Sexplib0.Sexp_conv_error.empty_list_invalid_sum error_source__019_
             sexp__018_
       | sexp__018_ ->
           Sexplib0.Sexp_conv_error.unexpected_stag error_source__019_
             sexp__018_ : Sexplib0.Sexp.t -> require)
    let _ = require_of_sexp
    let sexp_of_require =
      (function
       | Require arg0__036_ ->
           let res0__037_ = sexp_of_with_loc sexp_of_string arg0__036_ in
           Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "Require"; res0__037_]
       | Require_lib arg0__038_ ->
           let res0__039_ = sexp_of_with_loc sexp_of_string arg0__038_ in
           Sexplib0.Sexp.List [Sexplib0.Sexp.Atom "Require_lib"; res0__039_]
       | Require_extlib arg0__040_ ->
           let res0__041_ = sexp_of_with_loc sexp_of_extlib arg0__040_ in
           Sexplib0.Sexp.List
             [Sexplib0.Sexp.Atom "Require_extlib"; res0__041_] : require ->
                                                                   Sexplib0.Sexp.t)
  ...
  struct
    ...
    and sexp_of_module_kind =
      (function
       ...
       | MLX -> Sexplib0.Sexp.Atom "MLX" : module_kind -> Sexplib0.Sexp.t)
...
type mach_unit =
  ...
  | Unit_lib of mach_library [@@deriving sexp]
include
  struct
...
type 'a unit_validation =
  ...
  [@ocaml.doc
    " unit is fresh, no reconfiguration needed, but need to write state "]
  | Invalid
  [@ocaml.doc
    " unit is invalid (missing files or etc), dependent units need to be reconfigured "]
  | Changed of 'a
  [@ocaml.doc " unit has changed, it needs to be reconfigured "]
  | Toolchain_changed
  [@ocaml.doc " toolchain has changed, unit needs to be reconfigured "]
  | No_state
  [@ocaml.doc
    " no state exists for the unit, it needs to be configured from scratch "]
let validate_module config unit : Mach_module.t unit_validation=
  match unit with
  | Unit_module m ->
      (match file_stat m.path_ml with
       | None -> Invalid
       | Some path_ml_stat ->
...
let validate_lib config unit : Mach_library.t unit_validation=
  match unit with
  | Unit_lib lib ->
      let machlib_path = let open Filename in lib.path / "Machlib" in
      (match file_stat lib.path with
       | None -> Invalid
       | Some path_stat ->
           (match file_stat machlib_path with
            ...
            | Some machlib_stat ->
                let machlib_changed =
                  not (equal_file_stat machlib_stat lib.machlib_stat) in
                if not (Sys.file_exists machlib_path)
                then Invalid
                else
                  (let dir_changed =
                     not (equal_file_stat path_stat lib.path_stat) in
                   if machlib_changed
                   then Changed (Mach_library.of_path config lib.path)
                   else
                     if dir_changed
                     then
  ...
  ((state.toolchain).mach_executable_path <> mach_path) ||
    (((state.toolchain).ocaml_version <> toolchain.ocaml_version) ||
       (((state.toolchain).ocamlfind_version <> None) &&
          ((state.toolchain).ocamlfind_version <>
             (Lazy.force toolchain.ocamlfind).ocamlfind_version)))
let read path =
...
let read_mach_state config validate path =
  let filename =
    let open Filename in
      (Mach_config.build_dir_of config path) / "Mach.state" in
  match read filename with
  | None -> No_state
  | Some state ->
      if toolchain_changed config state
      then Toolchain_changed
...
let crawl config ~target_path =
  let exception Invalid_require  in
        ...
        then ()
          ...
          (Hashtbl.add visited path ();
           (try f ()
            with
            | Invalid_require ->
                (Hashtbl.remove visited path; raise_notrace Invalid_require))) in
    let units = ref [] in
    ...
    let rec visit_module path =
      (if_not_visited path) @@
        (fun () ->
           ...
           | Fresh m ->
               (match visit_requires m.requires with
    ...
    and visit_library path =
      (if_not_visited path) @@
        (fun () ->
           ...
           | Fresh lib ->
               (match visit_requires lib.requires with
    ...
    and visit_requires requires = List.iter visit_require (!! requires) in
    visit_require
      (let targe_path = Unix.realpath target_path in
       Mach_module.resolve_require config ~source_path:targe_path ~line:0
         targe_path);
